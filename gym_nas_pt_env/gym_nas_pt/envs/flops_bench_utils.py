...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    conv_per_position_flops = 2 * kernel_size * in_channels * out_channels / groups

    active_elements_count = batch_size * output_size

    if conv_module.__mask__ is not None:
        # (b, 1, s)
        flops_mask = conv_module.__mask__.expand(batch_size, 1, output_size)
